...
对于同时提供了静态工厂方法（[第 1 条][item1]）和构造器的不可变类，通常可以使用静态工厂方法来避免创建不必要的对象。例如，工厂方法Boolean.valueOf(String)比构造函数布尔(String)更可取，后者在Java 9中被弃用。构造函数必须每次调用一个新对象时都创建一个新对象，而工厂方法从来没有被要求这样做，实际上也不会。除了重用不可变对象之外，如果知道不会修改可变对象，还可以重用可变对象。 
