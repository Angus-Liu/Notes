...
通常应该重用一个对象，而不是在每次需要时创建一个新的功能等效的对象。重用可以更快也更时尚。如果一个对象是不可变的(项目17)，那么它总是可以被重用的。 
...
语句在每次执行时都创建一个新的字符串实例，这些对象创建都是不必要的。字符串构造函数(“比基尼”)的参数本身就是一个字符串实例，在功能上与构造函数创建的所有对象相同。如果这种用法发生在循环或频繁调用的方法中，则可以不必要地创建数百万个字符串实例。 
...
这个版本使用一个字符串实例，而不是每次执行时都创建一个新的。此外，可以保证对象将被运行在同一虚拟机中的任何其他代码重用，这些代码碰巧包含相同的字符串字面量[JLS, 3.10.5]。 
...
通常可以通过使用静态工厂方法(第1项)来避免创建不必要的对象，而不是在提供这两种方法的不可变类上使用构造函数。例如，工厂方法Boolean.valueOf(String)比构造函数布尔(String)更可取，后者在Java 9中被弃用。构造函数必须每次调用一个新对象时都创建一个新对象，而工厂方法从来没有被要求这样做，实际上也不会。除了重用不可变对象之外，如果知道不会修改可变对象，还可以重用可变对象。 
